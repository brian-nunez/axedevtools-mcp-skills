// Images-IGT step-4 batch review scraper: per row -> current YES/NO, radio
// indices, y-offset (for --scroll), raw text (accessible name, types, overlap).
...
for (const r of d.rows) {
  console.log(`#${String(r.row).padStart(2)} [${r.current}] yes=${r.yesIdx} no=${r.noIdx} y=${r.offsetY} :: ${r.raw}`);
}
await api.detach(s);
api.close();
process.exit(0);
